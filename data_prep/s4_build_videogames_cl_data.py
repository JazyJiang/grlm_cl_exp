"""
s4_build_videogames_cl_data.py
Generate Continual Learning SFT data for Video_Games dataset.
...
ID2META_FILE = "/workspace/jiangzhuosong/GRLM_0/in_domain/videogames/sum_data/videogames_id2meta.json"
...
def main():
    ...
    for t in range(4):
        ...
        save_json(sft_samples, f"{OUTPUT_DIR}/amazon_videogames_cl_D{t}_train.json")
        ...
        for cap in HISTORY_CAPS:
            ...
            save_json(sft_truncated, f"{OUTPUT_DIR}/amazon_videogames_cl_D{t}_train_h{cap}.json")

    # === Eval data ===
    print("\n=== Building Eval Data ===")
    for t in range(4):
        eval_data = build_eval_dt(period_users, id2meta, t)
        save_json(eval_data, f"{OUTPUT_DIR}/amazon_videogames_cl_D{t}_eval.json")

        # Truncated eval versions
        for cap in HISTORY_CAPS:
            truncated_eval = [truncate_eval_sample(s, cap) for s in eval_data]
            save_json(truncated_eval, f"{OUTPUT_DIR}/amazon_videogames_cl_D{t}_eval_h{cap}.json")
    ...
    for t in range(4):
        eval_file = f"{OUTPUT_DIR}/amazon_videogames_cl_D{t}_eval.json"
