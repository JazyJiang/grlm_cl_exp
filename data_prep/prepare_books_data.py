"""
Build Books dataset for GRLM in_domain pipeline.
Creates sequential_data.txt and books.item.json from memory_dev processed data.
"""
import numpy as np
import json
import csv
...
OUTPUT_DIR = "/workspace/jiangzhuosong/GRLM_0/in_domain/books/raw_data"

def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs("/workspace/jiangzhuosong/GRLM_0/in_domain/books/sum_data", exist_ok=True)
    ...
    # Build books.item.json (0-based -> 1-based)
    print("Building books.item.json...")
    item_json = {}
    ...
    with open(f"{OUTPUT_DIR}/books.item.json", "w", encoding="utf-8") as f:
        json.dump(item_json, f, ensure_ascii=False, indent=2)
    print(f"  Written {len(item_json)} items to books.item.json")
